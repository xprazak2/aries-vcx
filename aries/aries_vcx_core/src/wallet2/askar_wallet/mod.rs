...
use super::{DidData, RngMethod};

// use super::{RngMethod, key_alg::KeyAlg};
...
impl AskarWallet {
    ...
    fn local_key_to_pubkey(&self, local_key: &LocalKey) -> VcxCoreResult<String> {
        let public_bytes = local_key.to_public_bytes()?;
        Ok(std::str::from_utf8(&public_bytes)
            .map(Into::into)
            .map_err(|err| {
