use aries_askar::entry::{EntryTag as AskarEntryTag, Entry};
use async_trait::async_trait;

use crate::{errors::error::{VcxCoreResult, AriesVcxCoreError, AriesVcxCoreErrorKind}, wallet2::{RecordWallet, Record, SearchFilter}};
...
impl RecordWallet for AskarWallet {
    async fn add_record(&self, record: Record) -> VcxCoreResult<()> {
        ...
        let tags: Option<Vec<AskarEntryTag>> = Some(record.tags.into_iter().map(From::from).collect());
    ...
    async fn search_record(
        ...
        let tag_filter = search_filter.map(|filter| {
            match filter {
        ...
        let rs: Vec<_> = res.into_iter().map(TryFrom::try_from).collect::<Vec<Result<Record, _>>>().into_iter().collect::<Result<_, _>>()?;
        Ok(rs)
    }
}
