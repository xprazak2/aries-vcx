...
impl BaseAnonCreds for IndyCredxAnonCreds {
    ...
    async fn prover_store_credential(
        ...
        let tags_vec: Vec<(String, String)> = serde_json::from_value(tags)?;
        let tags = tags_vec.into_iter().map(From::from).collect();
    ...
    async fn prover_create_link_secret(
        ...
        let existing_record = wallet
            .get_record(link_secret_id, CATEGORY_LINK_SECRET.into())
            .await
