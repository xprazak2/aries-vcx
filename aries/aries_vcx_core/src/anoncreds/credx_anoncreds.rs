...
use crate::{
    ...
    wallet2::{
        BaseWallet2, EntryTags, Record, RecordBuilder, RecordUpdate, RecordUpdateBuilder,
        RecordWallet, SearchFilter,
    },
};
...
#[derive(Debug)]
struct WalletAdapter2(Arc<dyn BaseWallet2>);

#[async_trait]
impl RecordWallet for WalletAdapter2 {
...
impl BaseAnonCreds for IndyCredxAnonCreds {
    ...
    async fn issuer_create_credential_offer(
        ...
        let schema_id = SchemaId(schema.value);
    ...
    async fn prover_store_credential(
        ...
        let entry_tags: EntryTags = tags_map.into();

        let record = RecordBuilder::default()
            .name(credential_id.clone())
            .category(CATEGORY_CREDENTIAL.into())
            .value(record_value)
            .tags(entry_tags.into())
            .build()?;
