...
use crate::{
    ...
    wallet::{
        base_wallet::{record::Record, search_filter::SearchFilter, BaseWallet},
...
impl BaseAnonCreds for Anoncreds {
    ...
    async fn prover_store_credential(
        ...
        let mut tags = EntryTags::new(vec![
            EntryTag::new("schema_id", &schema_id.0),
            EntryTag::new("schema_issuer_did", schema_issuer_did.did()),
            EntryTag::new("schema_name", &schema_name),
            EntryTag::new("schema_version", &schema_version),
            EntryTag::new("issuer_did", issuer_did.did()),
            EntryTag::new("cred_def_id", &cred_def_id.0),
        ]);

        if let Some(rev_reg_id) = &credential.rev_reg_id {
            tags.add(EntryTag::new("rev_reg_id", &rev_reg_id.0));
        }
        ...
        for (raw_attr_name, attr_value) in credential.values.0.iter() {
            ...
            tags.add(EntryTag::new(&value_tag_name, &attr_value.raw));
